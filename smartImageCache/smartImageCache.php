...
if ($album_path) {
	...
	$_zp_admin_menu['overview']['subtabs']=array(
			gettext('Cache images')=>FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/smartImageCache.php?page=overview&tab=images'
	);
}
...
echo '<link rel="stylesheet" href="'.FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/smartImageCache.css" type="text/css"/>'.CR_LF;
echo '</head>'.CR_LF;
echo '<body>'.CR_LF;
printLogoAndLinks();
...
echo '						<input type="hidden" name="unique" id="unique" value="'.sha1(rand(1000,100000)).'"/>'.CR_LF;
echo '						<li><label>'.gettext('How many items has to be processed every cycle.').CR_LF;
if ($process) {
	echo '							<input type="hidden" name="quantity" id="quantity" value="'.$_REQUEST['quantity'].'"/>'.CR_LF;
	echo '							<input type="number" min="0" max="2000" value="'.$_REQUEST['quantity'].'" disabled/> '.gettext('<em>Value 0 (zero) means all images together in one chunk.</em> It can be used to review and control all the sizes together, when they are already cached.').CR_LF;
} else {
	echo '							<input type="number" min="0" max="2000" name="quantity" id="quantity" value="'.$_REQUEST['quantity'].'" /> '.gettext('<em>Value 0 (zero) means all images together in one chunk.</em> It can be used to review and control all the sizes together, when they are already cached.').CR_LF;
}
echo '						</label></li>'.CR_LF;
echo '						<li><label>'.CR_LF;
if ($process) {
	...
	echo '							<input type="checkbox" name="show" id="show" value="true" '.($_REQUEST['show']?'checked':'').'/> '.gettext('Show already cached thumbs.').' '.gettext('(Activate only if needed, otherwise you can overcharge client and server without a real benefit)').CR_LF;
}
echo '						</label></li>'.CR_LF;
echo '						<br>'.CR_LF;
echo '						<li><label>'.CR_LF;
...
if ($process) {
	...
	echo '							<input type="number" min="0" max="120" value="'.$_REQUEST['timer'].'" disabled/> '.gettext('secs').CR_LF;
} else {
	echo '							<input type="number" min="0" max="120" name="timer" id="timer" value="'.$_REQUEST['timer'].'" '.(!$_REQUEST['auto']?'disabled':'').'/> '.gettext('secs').CR_LF;
}
echo '						</label></li>'.CR_LF;
echo '					</ol>'.CR_LF;
						$button=false;
...
echo '								<ul>'.CR_LF;
echo '									<li><img src="'.FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/images/time.png" height="20" alt="time" title="time"> '.gettext('Automation:').' <b><span class="imagecaching_auto">'.($_REQUEST['auto']?'ON':'OFF').'</span></b> '.($_REQUEST['auto']?'(':'').'<span class="imagecaching_pause">'.($_REQUEST['auto']?$_REQUEST['timer']:'').'</span>'.($_REQUEST['auto']?' secs waiting)':'').'</li>'.CR_LF;
...
echo '									<li><img src="'.FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/images/album.png" height="20" alt="album" title="album"> '.gettext('Albums processed:').' <b><span class="imagecaching_albumcount">-</span></b> / <b><span>'.smartImageCache::$albums_total.'</span></b></li>'.CR_LF;
echo '									<li><img src="'.FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/images/image.png" height="20" alt="image" title="image"> '.gettext('Items processed:').' <b><span class="imagecaching_imagecount">-</span></b> / <b><span>'.smartImageCache::$images_total.'</span></b> (max. '.$_REQUEST['quantity'].' items every chunk)</li>'.CR_LF;
echo '									<li><img src="'.FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/images/cache.png" height="20" alt="cached" title="cached"> '.gettext('Cache sizes processed:').' <b><span class="imagecaching_sizecount">-</span></b> / <b><span>'.smartImageCache::$imagesizes_total.'</span></b> (max. '.($_REQUEST['quantity']*smartImageCache::$imagesizes_sizes).' cache sizes every chunk)'.CR_LF;
...
echo '											<li><img src="'.FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/images/media.png" height="20" alt="media" title="media"> '.gettext('Multimedia without Sidecars: ').'<span class="imagecaching_imagesizes_media">'.smartImageCache::$imagesizes_media.'</span> '.gettext('sizes not produced').'</li>'.CR_LF;
echo '											<br>'.CR_LF;
...
echo '									<button class="tooltip" type="submit" id="continue_button1" title="'.gettext('Continue').'" onclick="next();">'.CR_LF;
...
echo '									<a title="'.gettext('Reload').'" id="reload_button1" href="javascript:history.go(0);">'.CR_LF;
...
echo '											<a title="'.gettext('New cache size selection').'" href="'.FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/smartImageCache.php?page=overview&tab=images&album='.$album_path.'">'.CR_LF;
...
echo '									document.getElementById(\'continue_button1\').remove();'.CR_LF;
echo '									document.getElementById(\'continue_button2\').remove();'.CR_LF;
echo '									document.getElementById(\'pause_button1\').remove();'.CR_LF;
echo '									document.getElementById(\'pause_button2\').remove();'.CR_LF;
echo '									document.getElementById(\'reload_button1\').remove();'.CR_LF;
echo '									document.getElementById(\'reload_button2\').remove();'.CR_LF;
...
echo '									document.getElementById(\'pause_button1\').disabled=true;'.CR_LF;
echo '									document.getElementById(\'pause_button2\').disabled=true;'.CR_LF;
echo '								}'.CR_LF;
echo '							</script>'.CR_LF;
							}
echo '						<p class="buttons buttons_cachefinished clearfix">'.CR_LF;
echo '							<button class="tooltip" type="submit" id="continue_button2" title="'.gettext('Continue').'" onclick="next();">'.CR_LF;
...
echo '							<a title="'.gettext('Reload').'" id="reload_button2" href="javascript:history.go(0);">'.CR_LF;
echo '								<img src="'.FULLWEBPATH.'/'.USER_PLUGIN_FOLDER.'/smartImageCache/images/reload.png" height="20" alt="reload">'.CR_LF;
echo '							  	<strong>'.gettext("Reload chunk").'</strong>'.CR_LF;
echo '							</a>'.CR_LF;
echo '						</p>';
